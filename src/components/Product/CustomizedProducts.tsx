...
const CustomizeProducts = ({
  ...
  const presentVarient = (var_opt: string): boolean => {
    return !productOptions?.find((product_var) => 
  ...
  const handleClick = (var_name: string, var_opt: string) => {
    setSelectedOptions((prevOptions) => {
      ...
      const index = options.findIndex(option => Object.keys(option)[0] === var_name);
      ...
      if (index > -1) {
        ...
        if (isPresent) {
          ...
          optionCopy[var_name] = optionCopy[var_name].filter(opt_name => opt_name !== var_opt);
        } else {
          // If the option is not present, add it
          optionCopy[var_name] = [...optionCopy[var_name].filter(opt_name => opt_name !== var_opt),var_opt]
        }
  ...
  const isClicked = (var_name: string, var_opt: string) => {
    const index = selectedOptions.findIndex(option => Object.keys(option)[0] === var_name); 
    if(index > -1 ){
                  ...
                  <li
                    ...
                    style={{
                      ...
                      presentVarient(option.value)
                        ...
                        ? "#FBCFE8"
                        : "white",
                      color: isClicked(varient.name, option.value)|| presentVarient(option.value) ? "white" : "#f35c7a",
