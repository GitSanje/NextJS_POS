                            ...
                            <div className="px-4 py-6 sm:p-6 lg:p-8">
                                <div className="grid grid-cols-2 gap-6 md:grid-cols-4 lg:grid-cols-1">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-sm font-bold text-gray-900 mt-0.5">{formatOrderDate(order?.orderDate)}</p>
                                    </div>

                                    <div>
                                        <p className="text-sm font-medium text-gray-500">Total Amount</p>
                                        <p className="text-sm font-bold text-gray-900 mt-0.5">${totalamount}
                                           </p>
                                    </div>
                            ...
                            <ul className="space-y-7">
                                {
                                    ...
                                    <div className="flex flex-col justify-between flex-1 ml-5">
                                        <div className="sm:grid sm:grid-cols-2 sm:gap-x-5">
                                            <div>
                                                <p className="text-base font-bold text-gray-900">{ cart.product.name}</p>
                                                <p className="mt-1.5 text-sm font-medium text-gray-500">{
                                                    cart.variants.length > 0 ?  cart.variants.map((var_opt) => {
                                            ...
                                            <div className="mt-4 sm:mt-0">
                                                <p className="text-base font-bold text-left text-gray-900 sm:text-right">${
                                                    cart.variants.length > 0 ? cart.variants.find(var_product => 
                                                        var_product.variant.name === 'Size')?.salePrice || cart.product.salePrice:
                                                    cart.product.salePrice}</p>
                                            </div>
                                        </div>
